		...
		link: function(scope){
			...
			scope.validQuantityAddToOrder = function(value, lineItem){
				...
				if(product.IsVariantLevelInventory && !variant){
					...
					var qtyAvail = product.IsVariantLevelInventory ? variant.QuantityAvailable : (lineItem.Product.OrderLimit || lineItem.Product.OrderLimit == 0) ? (lineItem.Product.OrderLimit + (lineItem.OriginalQuantity || 0)) : (lineItem.QuantityAvailable + (lineItem.OriginalQuantity || 0));
