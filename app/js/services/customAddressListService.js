...
four51.app.directive('largeshippingaddresslist', function() {
    var obj = {
        ...
        template: '<style>.dropdown-menu { font-size: 11px; }</style>' +
            ...
            'typeahead="address as (address.AddressName + \' \' + (address.FirstName || \'\') + \' \' + (address.LastName || \'\') + \' \' + (address.Street1 || \'\') + \' \' + (address.Street2 || \'\') + \' \' + (address.City || \'\') + \' \' + (address.State || \'\') + \' \' + (address.Zip || \'\')) for address in addresses | largeaddress:$viewValue | filter:{IsShipping:true} | limitTo:15"' +
...
four51.app.directive('largebillingaddresslist', function() {
    var obj = {
        ...
        template: '<style>.dropdown-menu { font-size: 11px; }</style>' +
            ...
            'typeahead="address as (address.AddressName + \' \' + (address.FirstName || \'\') + \' \' + (address.LastName || \'\') + \' \' + (address.Street1 || \'\') + \' \' + (address.Street2 || \'\') + \' \' + (address.City || \'\') + \'' +  '\' + (address.State || \'\') + \' \' + (address.Zip || \'\')) for address in addresses | largeaddress:$viewValue | filter:{IsBilling:true} | limitTo:15"' +
            'class="form-control"><i class="fa fa-map-marker"></i></div>',
        controller: ['$scope', function($scope) {
